...
fig = plt.figure(figsize=(7.20472, 1.9))
for task_index, task in enumerate(df['task']):
    ...
    ax.set_title('choices13k' if task_index == 0 else 'Intertemporal choice', fontsize=7)

    ax.set_ylabel('Negative log-likelihood', labelpad=20)
    ...
    ax.axs[-2].containers[0][4].set_alpha(0.8)
    ax.axs[-2].containers[0][5].set_alpha(0.8)
    ax.axs[-2].containers[0][4].set_hatch('///')
    ax.axs[-2].containers[0][5].set_hatch('///')

fig.text(0.07, 0.925, 'a', fontsize=8, weight='bold')
fig.text(0.493, 0.925, 'b', fontsize=8, weight='bold')

#sns.despine()
#plt.tight_layout()
plt.savefig('figures/fig5.jpg', bbox_inches='tight', dpi=300)
plt.show()

#!/bin/bash
#SBATCH --job-name=context_free_centaur_eval        # Job name
#SBATCH --output=context_free_centaur_eval_%j.out   # Standard output log (%j is job ID)
#SBATCH --error=context_free_centaur_eval_%j.err    # Standard error log (%j is job ID)
...
echo "Starting Context-Free Centaur Evaluation (Refactored Pipeline)..."
echo "Model: marcelbinz/Llama-3.1-Centaur-70B-adapter"
echo "Task: ruggeri2022globalizability"
echo "Pipeline: Refactored evaluation with shared core functions"
echo "Packages: unsloth + transformers + trl (same as original)"
echo "Difference: Only prompts changed to context-free (no behavioral history)"
...
srun python evaluate_history_only_centaur.py \
